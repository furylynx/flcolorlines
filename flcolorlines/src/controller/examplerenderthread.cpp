...
void ExampleRenderThread::drawNormalField(){
...
//                        else
            ...
            float *quadcolor = QColorToQuadcolor(c);
            glMaterialfv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE, quadcolor/*quadcolor*/);
            ...
            if (!field->getBallAt(i,j).isNull && !field->getBallAt(i,j).outOfField){
                ...
                glBegin(GL_QUADS);
                glMaterialfv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE, QColorToQuadcolor(c)/*quadcolor*/);
                glMateriali(GL_FRONT, GL_SHININESS, 96);
...
void ExampleRenderThread::drawNormalFieldWithAnimation(){
...
void ExampleRenderThread::drawIdleSequence(){

    //begin actual drawing of the element

    long ms = timeOperator->getTimeSinceBegin();

    float *quadcolor = QColorToQuadcolor(Qt::darkRed);
    glMaterialfv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE, quadcolor);
    glMateriali(GL_FRONT, GL_SHININESS, 96);
    delete quadcolor;
...
//    glRotatef(-25, 1.0, 1.0, 0.0);
    ...
    quadcolor = QColorToQuadcolor(Qt::darkBlue);
    glMaterialfv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE, quadcolor);
    glMateriali(GL_FRONT, GL_SHININESS, 96);
    delete quadcolor;
    ...
    quadcolor = QColorToQuadcolor(Qt::darkYellow);
    glMaterialfv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE, quadcolor);
    glMateriali(GL_FRONT, GL_SHININESS, 96);
    delete quadcolor;
...
void ExampleRenderThread::drawInitialAnimation(){
...
void ExampleRenderThread::calcAnimationTranslation(model::GameField *field, int i, int j, float dist, float *out_x, float *out_y, float *out_z, float *out_color, float *out_size){
    timespec now;
    clock_gettime(1, &now);
...
float ExampleRenderThread::quadraticNewBallFunction(float d, long t_millisec){
...
float ExampleRenderThread::quadraticColorFunction(long t_millisec){
...
void ExampleRenderThread::movementPositionFunction(long t_millisec, std::vector<model::FieldPosition> path, float *out_x, float *out_y){
...
void ExampleRenderThread::setSelectedField(int face){
...
void ExampleRenderThread::setHoveredField(int face){
...
float * ExampleRenderThread::QColorToQuadcolor(QColor c) const{

    float *quadcolor = new float[4];
    quadcolor[0] = c.redF();
    quadcolor[1] = c.greenF();
    quadcolor[2] = c.blueF();
    quadcolor[3] = c.alphaF();

    return quadcolor;
}

void ExampleRenderThread::beginInitialAnimation(){
